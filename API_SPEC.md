...
설치·시공 범위는 출입문 형태·개수 / 사용 중인 전자기기 종류·개수 / 전력구조 등
...
**공통 필수**: `photos`(§5.4.1) · `floor`(층수, 자유 텍스트) ·
`preferredPeriod`(희망 시공 시기) · `managerName` · `managerPhone` / **선택**: `etc`(추가 요청사항)

> 2026-07-29 2차 정리: `area`(센터 평수) 문항 **폐지**, `floor`에서 **엘리베이터 유무 제거**(층수만 수집).

| key | 기능별 문항 |
|---|---|
| `access_control` | 추가 문항 없음 — **현장 사진 4칸(필수) + 1칸(선택)이 판단 근거 전부** |
| `iot_control` | `targetDevices` **필수** — 제어 대상 기기 다중선택 + **기기별 수량만**(제어방식 select 폐지)<br>옵션: 조명 / 냉난방기(에어컨·히터) / 환기설비 / 급탕·온수 / 제습·가습기 / 음향설비 / 간판·사인 / **기타**<br>`기타` 선택 시 기기 이름을 자유 입력받아 **입력한 이름으로 치환**해 전송(`제빙기 2개`). 이름이 비면 제출 불가.<br>📏 기기 이름 **최대 17자**(`OTHER_NAME_MAX_LENGTH`) — 320px에서 한 줄에 다 보이는 한글 글자수 실측값. **서버도 동일하게 17자 초과를 거부**할 것<br>⚠️ `기타`는 옵션 배열 **맨 뒤 고정** — `hvac` 슬롯 `reqIf` 가 인덱스 1(냉난방기)을 참조 |
